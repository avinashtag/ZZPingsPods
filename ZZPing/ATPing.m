...
-(id)copyWithZone:(NSZone *)zone {
    Ping *copy = [[[self class] allocWithZone:zone] init];
...
+ (ATPing *)pingWithHostName:(NSString *)hostName{
    return [[ATPing alloc] initWithHostName:hostName address:nil];
}

+ (ATPing *)pingWithHostAddress:(NSString *)hostAddress{
    return [[ATPing alloc] initWithHostName:NULL address:hostAddress];
}
...
// ICMP socket.  It just redirects the call to Objective-C code.
{
    ...
    obj = (__bridge ATPing *) info;
    assert([obj isKindOfClass:[ATPing class]]);
...
// messages waiting on the socket.
{
    ...
    if (bytesRead > 0) {
        ...
        [self validatePacket:packet :^(BOOL success) {
            ...
            NSNumber *key = @(seqNo);
            Ping *ping = (Ping *)self.pings[[key stringValue]];
